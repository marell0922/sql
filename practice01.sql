...
select gender, count(*) as '수' from employees group by gender;
-- 문제4.
...
select count(distinct(emp_no)) as '현재 부서 매니저 수' from dept_manager where to_date = '9999-01-01';
-- 문제7.
...
-- select emp_no, salary from salaries where salary >=120000;
-- 문제9.
-- 어떤 직책들이 있나요? 중복 없이 이름이 긴 순서대로 출력해 보세요.
select distinct title from titles order by length(title) desc; 
-- 문제10
-- 현재 Engineer 직책의 사원은 총 몇 명입니까?
select count(*) as '총 인원수' from titles where title='Engineer' and to_date = '9999-01-01';
-- 문제11
-- 사번이 13250(Zeydy)인 지원이 직책 변경 상황을 시간순으로 출력해보세요.
select title, from_date, to_date from titles where emp_no=13250 order by from_date desc; --
